...
def generate_power_curves_single_profile(config_filename):
    ...
    export_to_csv_single_profile(pc.wind_speeds, vw_cut_out, p_cycle, pc.x_opts, n_cwp, pc.optimization_details, control_mode)
...
def load_power_curve_single_profile_results_and_plot_trajectories(config_filename,\
    ...
    if control_mode == 'hybrid':
        labels = [r'$v_{r, RO}$', r'$F_{T, RI}$', r'$\theta_{avg}$', r'$\theta_{rel}$', r'$\phi_{max}$',\
                                r'$\Delta L$', r'$L_{min}$']
