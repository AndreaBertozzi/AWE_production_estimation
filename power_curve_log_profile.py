...
def generate_power_curves_logprofile(x0, opt_settings, sys_props, reduce_x, vw_cut_in = 5, vw_cut_out = 18, vw_interval = 1):
    ...
    wind_speeds = np.arange(vw_cut_in, vw_cut_out-1, vw_interval)
    ...
    op_cycle_pc = OptimizerCycle(cycle_sim_settings_pc, sys_props, env, reduce_x,
...
if __name__ == "__main__":
    ...
    opt_settings = {'maxiter': 30, 'iprint': 2, 'ftol': 1e-3, 'eps': 1e-6}
    ...
    pc = generate_power_curves_logprofile(x0_force, opt_settings, sys_props, reduce_x = np.array([0, 1, 2, 4, 5, 6]), vw_cut_in = 5, vw_cut_out = 18, vw_interval = 1)
